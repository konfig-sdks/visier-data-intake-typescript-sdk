...
import { readableStreamToString, VisierDataIntakeError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierDataIntakeError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierDataIntakeError(e, e.response?.data)
                }
                if (innerError instanceof VisierDataIntakeError) {
                    // Got "VisierDataIntakeError" from the above try block
                    throw innerError;
                }
